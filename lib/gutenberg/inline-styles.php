...
add_action( 'wp_enqueue_scripts', 'sixseed_custom_gutenberg_css' );
/**
 * Outputs front-end inline styles based on colors declared in config/appearance.php.
 *
 * @since 1.0.0
 */
function sixseed_custom_gutenberg_css() {
...
CSS;
	...
	$css .= sixseed_inline_font_sizes();
	$css .= sixseed_inline_color_palette();

	wp_add_inline_style( genesis_get_theme_handle() . '-gutenberg', $css );

}

add_action( 'enqueue_block_editor_assets', 'sixseed_custom_gutenberg_admin_css' );
/**
 ...
 * @since 1.0.0
 */
function sixseed_custom_gutenberg_admin_css() {
 ...
 */
function sixseed_inline_font_sizes() {
 ...
 */
function sixseed_inline_color_palette() {

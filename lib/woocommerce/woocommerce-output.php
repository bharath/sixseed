 ...
 * Sixseed.
 *
 * This file adds the WooCommerce styles and the Customizer additions for WooCommerce to the Sixseed Theme.
 *
 * @package Sixseed
 * @author  Bharath
...
add_filter( 'woocommerce_enqueue_styles', 'sixseed_woocommerce_styles' );
/**
 ...
 */
function sixseed_woocommerce_styles( $enqueue_styles ) {

	$enqueue_styles[ genesis_get_theme_handle() . '-woocommerce-styles' ] = [
		'src'     => get_stylesheet_directory_uri() . '/lib/woocommerce/sixseed-woocommerce.css',
		'deps'    => '',
...
add_action( 'wp_enqueue_scripts', 'sixseed_woocommerce_css' );
/**
 ...
 */
function sixseed_woocommerce_css() {
	...
	$color_link   = get_theme_mod( 'sixseed_link_color', $appearance['default-colors']['link'] );
	$color_accent = get_theme_mod( 'sixseed_accent_color', $appearance['default-colors']['accent'] );
	...
	',
		$color_accent,
		sixseed_color_contrast( $color_accent )
	) : '';

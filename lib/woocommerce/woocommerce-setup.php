 ...
 * Sixseed.
 *
 * This file adds the required WooCommerce setup functions to the Sixseed Theme.
 *
 * @package Sixseed
 * @author  Bharath
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'sixseed_woocommerce_breakpoint' );
/**
 ...
 */
function sixseed_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'sixseed_default_products_per_page' );
/**
 ...
 */
function sixseed_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'sixseed_woocommerce_pagination' );
/**
 ...
 */
function sixseed_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'sixseed' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'sixseed' ) );

	return $args;

}

add_action( 'after_switch_theme', 'sixseed_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 1.0.0
 */
function sixseed_woocommerce_image_dimensions_after_theme_setup() {
	...
	sixseed_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'sixseed_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function sixseed_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 1.0.0
 */
function sixseed_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'sixseed_gallery_image_thumbnail' );
/**
 ...
 */
function sixseed_gallery_image_thumbnail( $size ) {

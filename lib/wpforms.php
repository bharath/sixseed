<?php
/**
 * Bharath WPForms helper functions.
 *
...
/**
 * Creates a WPForms form if one added by a Bharath theme does not exist.
 *
 * @since 1.0.0
 *
 * @return int|null ID of form if one exists or gets created. Null if form creation fails or WPForms is inactive.
 */
function bharath_maybe_create_wpforms_form() { // phpcs:ignore -- bharath prefix for functions shared between themes.
	...
	$new_form_id = wpforms()->form->add(
		esc_html__( 'Simple Contact Form', 'sixseed' ),
		[],
		[
			'template' => 'contact',
			'builder'  => false,
 ...
 */
function bharath_insert_contact_form( $content, $imported_posts ) { // phpcs:ignore -- bharath prefix for functions shared between themes.

	$form_id = bharath_maybe_create_wpforms_form();
